...
/// `calculateTransactionFees()` returns minimum fees needed for the transaction to be
...
#[wasm_bindgen(js_name = calculateTransactionFees)]
pub fn calculate_transaction_fees(network_id: NetworkIdT, tx: &TransactionT) -> Result<Option<u64>> {
    let tx = Transaction::try_cast_from(tx)?;
    let network_id = NetworkId::try_owned_from(network_id)?;
    let consensus_params = Params::from(network_id);
    let network_params = NetworkParams::from(network_id);
